<template>
  ...
  <div class="border border-solid border-slate-600 rounded-md mb-5">
    ...
    <Table
      ...
      :fields="['dish', 'function', 'note', 'cooking', 'ingredient']"
      :labels="['name', 'function', 'note', 'cooking']"
    />
  </div>
  <FormEditMeal
    v-if="activeAdd"
      ...
      try {
        ...
        this.item = this.item.map((value, index) => {
          ...
          for (var value1 of value.mealTicket) {
            console.log("cc");
            console.log(value1);
            console.log(value1.evaluate.like);
            console.log(value1.evaluate.like == true);
        ...
        this.item.grade = this.item.grade.map((value, index) => {
          return {
            _id: value._id,
            name: value.name,
            checked: true,
