...
import Table from "../../../../components/TableEditCDI.vue";
...
export default {
  ...
  methods: {
    formatTable() {
      this.items = this.classes.cdi.filter((item, index) => {
        return item.month == this.monthValue;
      });
    ...
    async editCDI() {
      ...
      if (this.monthValue != null) {
        for (let value of this.items) {
          if (value.editCDI && this.monthValue && this.classId) {
            console.log("start");
            try {
              console.log(value._id);
              await CDI.update(value._id, {
                height: value.height,
                weight: value.weight,
                health: value.health,
                roses: value.roses,
        ...
        if (check == true) {
          ...
          Swal.fire({
            background: this.background,
            color: "white",
            text: "Please select a child to edit to the CDI.",
            icon: "error",
          });
        }
