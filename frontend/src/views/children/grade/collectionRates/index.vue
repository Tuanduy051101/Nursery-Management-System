...
export default {
  ...
  methods: {
    ...
    async getAll() {
      try {
        this.items = await CollectionRates.getAll();
      } catch (error) {
        console.error(error);
      }
    },
    ...
    async get() {
      try {
        this.item = await CollectionRates.get(this.deleteValue);
      } catch (error) {
        console.error(error);
      }
    },
    ...
    formatTable() {
      this.items = this.items.map((item, index) => {
        return {
          _id: item._id,
          money: this.formatMoney(item.money),
    ...
    async create() {
      try {
        const alert = await CollectionRates.create(this.itemAdd);
        if (alert.error) {
          Swal.fire({
    ...
    async delete() {
      const option = Swal.fire({
        ...
        html:
          ...
          this.item.tuitionFees.name +
          " - " +
          this.formatMoney(this.item.money) +
          "</span>" +
          '"' +
          "</p>",
      ...
      if ((await option).isConfirmed) {
        try {
          await CollectionRates.delete(this.deleteValue);
          Swal.fire({
            background: this.background,
            color: "white",
