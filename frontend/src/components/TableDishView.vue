    ...
    <table class="border-collapse w-full table-auto">
      ...
      <tbody>
        ...
        >
          ...
          <td class="p-2">
            <span class="float-left">{{ item.foodstuff.name.length > 0 ? item.foodstuff.name : 'empty' }}</span>
          </td>
          <td class="p-2">
            <span class="float-left">{{ item.foodstuff.function.length > 0 ? item.foodstuff.function : 'empty' }}</span>
          </td>
          <td class="p-2">
            <span class="float-left">{{ item.amount.length > 0 ? item.amount : 'empty' }}</span>
          </td>
          <td class="p-2">
            <span class="float-left">{{ item.foodstuff.making.length > 0 ? item.foodstuff.making : 'empty' }}</span>
          </td>
          <td class="p-2">
            <span class="float-left">{{ item.foodstuff.note.length > 0 ? item.foodstuff.note : 'empty' }}</span>
          </td>
        </tr>
      </tbody>
    </table>
