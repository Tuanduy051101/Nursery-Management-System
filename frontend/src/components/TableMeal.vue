    ...
    <table class="border-collapse w-full" :class="mode == 0 || mode == `auto` ? 'table-auto' : 'table-fixed' ">
      ...
      <tbody>
        ...
        >
          ...
          <!-- dish -->
          <td class="p-2">
            <span class="flex" v-for="(value, index1) in item.dish" >{{ index1 + 1 }}. {{ value.name }}</span>
          </td>
          <!-- datetime -->
          <td class="p-2">
            <!-- <span class="flex">{{ item.date }}</span> -->
            <span class="">{{ item.timeStart }} - {{ item.timeEnd }}</span>
          </td>
          <!-- amount of meal tickets -->
          <!-- datetime -->
          <td class="p-2">
            <span class="float-left">{{ item.amountOfMealTickets }}</span>
          </td>
          <td class="p-2">
            <span class="float-left">{{ item.like }}</span>
          </td>
          <td class="p-2">
            <span class="float-left">{{ item.unlike }}</span>
          </td>
          <td class="p-2">
            <span class="float-left">{{ item.allergy }}</span>
          </td>
          <td class="p-2">
            <router-link
              :to="{ name: actionList[0], params: { id: item._id } }"
