<template>
          ...
          <div>
            ...
            <div class="flex flex-col text-slate-900">
              <label for="" class="flex mb-2 items-center"
                >Ngày điểm danh<span class="text-red-500 text-3xl relative ml-0.5"
                  >*</span
                ></label
              >
              <input
                ...
                name="name"
                type="date"
                :class="!itemAdd.date ? 'border-red-500' : 'border-slate-300'"
                autocomplete="off"
                disabled
              />
              <span v-if="!itemAdd.date" class="text-red-500 mt-2 ml-0.5 text-sm"
            ...
            <div class="flex flex-col text-slate-900">
              ...
              class="-mt-2.5"
            />
              <span v-if="!itemAdd.session" class="text-red-500 mt-2 ml-0.5 text-sm"
                >Trường này là bắt buộc.</span
              >
            </div>
            <button
  ...
  const sessions = ref([]);
  onBeforeMount(async () => {
    sessions.value = await http_getAll(Session);
