    ...
    <table class="border-collapse w-full" :class="mode == 0 || mode == `auto` ? 'table-auto' : 'table-fixed' ">
      ...
      <tbody>
        ...
        >
          ...
          <td class="p-2">
            <span class="float-left">{{ item.child }}</span>
          </td>

          <td class="p-2">
            <span v-for="(value, index1) in item.dish" class="flex">{{ index1 + 1 }}. {{ value.name }}</span>
          </td>
          <td class="p-2">
            <input :checked="item.like" class="ml-2.5" type="checkbox" name="" id="" v-model="item.like">
          </td>
          <td class="p-2">
            <input :checked="item.allergy" class="ml-7" type="checkbox" name="" id="" v-model="item.allergy">
          </td>
          <td class="p-2">
            <textarea v-model="item.note" class="px-2 py-1 bg-inherit border border-solid outline-none">{{ item.note }}</textarea>
          </td>
          <td class="p-2">
            <router-link
