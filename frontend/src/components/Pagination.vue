<template>
    <div class="flex justify-between items-center py-5 px-2">
        ...
        <div class="flex items-center text-md">
            <span class="rounded-md text-slate-600 w-8 h-8 flex items-center justify-center border border-solid border-slate-600 cursor-pointer hover:border-slate-300 hover:text-slate-300"
            ...
            >
                <span class="rounded-md w-8 h-8 flex items-center justify-center cursor-pointer my-1 mx-2"
                v-for="(numberOfPage, index) in numberOfPages" :key="numberOfPage" 
                @click="updateCurrentPage(numberOfPage)"
                :class="[index === (currentPage - 1) ? 'text-slate-300 border-slate-300 border border-solid' : 'text-slate-600 border border-solid hover:text-slate-300 hover:border-slate-600']"
                >
                    {{numberOfPage}}
                </span>
            </div>
